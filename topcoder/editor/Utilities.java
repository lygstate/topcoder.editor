...
public class Utilities {
	...
	public static final String replaceUserDefined(
			String source,
		...
		for (Iterator<String> itr = userDefinedTags.keySet().iterator(); itr.hasNext();) {
			try {
				...
				System.out
						.println("Error in userDefinedTags - either the tag or it's source was not a String type");
			}
		}
	...
	public static final String replaceAll(String source, String text,
			String newText) {
		int pos = -1;
		while (true) {
			...
			if (pos < 0)
				break;
			source = source.substring(0, pos) + newText
					+ source.substring(pos + text.length());
		}
		return source;
	}
		...
		try {
			while (true) {
				...
				temp = replaceAll(temp, "$LINENUMBER$",
						String.valueOf(in.getLineNumber()));
				temp = replaceAll(temp, "$NEXTLINENUMBER$",
						String.valueOf(in.getLineNumber() + 1));
				str.append(temp);
