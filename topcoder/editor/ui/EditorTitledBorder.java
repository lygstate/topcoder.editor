...
public class EditorTitledBorder extends TitledBorder {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -8273778443458166531L;
	...
	public void paintBorder(Component c, Graphics g, int x, int y, int width,
		...
		Rectangle grooveRect = new Rectangle(x + 3, y + 3, width - 6,
				height - 6);
		Font font = g.getFont();
		...
		case 3:
			...
			break;
		case 4:
			this.textLoc.y = (grooveRect.y + grooveRect.height - (insets.bottom
					+ descent + 3));
			break;
		case 5:
			grooveRect.height -= fontHeight / 2;
			this.textLoc.y = (grooveRect.y + grooveRect.height - descent + (ascent
					+ descent - insets.bottom) / 2);
			break;
		case 6:
			grooveRect.height -= fontHeight;
			this.textLoc.y = (grooveRect.y + grooveRect.height + ascent + 3);
		}
		...
		if (border != null) {
			if ((titlePos == 2) || (titlePos == 5)) {
				...
				clipRect.setBounds(saveClip);
				if (computeIntersection(clipRect, this.textLoc.x + stringWidth,
						0, width - stringWidth - this.textLoc.x, height)) {
					g.setClip(clipRect);
					border.paintBorder(c, g, grooveRect.x, grooveRect.y,
							grooveRect.width, grooveRect.height);
				}
				...
				clipRect.setBounds(saveClip);
				if (titlePos == 2) {
					if (computeIntersection(clipRect, this.textLoc.x,
							grooveRect.y + insets.top, stringWidth, height
									- grooveRect.y - insets.top)) {
						g.setClip(clipRect);
						border.paintBorder(c, g, grooveRect.x, grooveRect.y,
								grooveRect.width, grooveRect.height);
					}
